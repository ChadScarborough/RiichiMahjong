...
namespace RMU.Games.Scoring
{
    internal static class FuCalculator
    {
        ...
        {
            if (completeHand.GetCompleteHandType() == SEVEN_PAIRS) return 25;
            if (completeHand.GetCompleteHandType() == THIRTEEN_ORPHANS) return 0;
            int fu = 20;
        ...
        {
            if (winningCallType == TSUMO)
                return 2;
            if (winningCallType == RON && (completeHand.IsOpen() == false))
                return 10;
            return 0;
        }
        ...
        {
            if (Functions.AreWindsEquivalent(windTile, seatWind))
                return 2;
            return 0;
        }
